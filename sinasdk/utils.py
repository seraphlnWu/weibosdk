...
def parse_search_datetime(str):
    ...
    return datetime(*(time.strptime(str, '%a %b %d %H:%M:%S +%f %Y')[0:7]))
...
def build_parameters(*args, **kargs):
    parameters = {}
    for idx, arg in enumerate(args):
        try:
            parameters[allowed_param[idx]] = convert_to_utf8_str(arg)
        except IndexError:
    ...
    for k, arg in kargs.items():
        ...
        parameters[k] = convert_to_utf8_str(arg)
